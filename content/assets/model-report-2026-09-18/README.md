...
页面为自包含 HTML，内嵌同一份 JSON 和 21 张 matplotlib SVG；公式使用 MathML，无需外部 CDN 或数学排版库。`figures/` 另存同一批图的 SVG 与 PNG，供单独查看和引用。主日报与展示版必须同时存在，才能进入看板索引。
...
图表样式集中在 `scripts/report_charts.py`：刻度使用 nice-number 分档，内存、核数、耗时等非负量的坐标轴从 0 起；每张内联 SVG 的元素 id 加图表前缀，避免同一页面内冲突。
...
RankIC 分解：每只股票对当日 Spearman 相关的贡献等于分数秩偏差乘收益秩偏差，除以同一归一化分母；按十分组求和后十组之和精确等于当日 RankIC，再对日期等权。组内 RankIC 是只在该组内部重新排名后的相关，与贡献不是同一个量，不可相加。十分组的日均收益误差棒为 Newey-West 标准误（Bartlett 核，lag = 4）乘 1.96，衡量均值估计精度，不是个股收益范围。
