...
交付文件为 `content/daily/2026-09-06.html`，全部图表和汇总数据内嵌，约 10 MB，可直接离线阅读。站点通过同日期 Markdown 日报与 `.show.html` 展示入口发现报告；展示版从 `content/assets/cancel-phase-2026-09-06/` 加载同源图片。
...
`report.json` 保存汇总数字、逐日日度 IC 和报告审计信息，不含股票代码或逐笔行情。102 张静态图使用 Matplotlib 绘制并保存为 WebP；各因子的散点图与 OLS 使用全部成对有效股票日，十分组交互图使用同一份已核验的区间汇总数据，可导出 SVG 与 CSV。
...
数值测试覆盖并列值、缺失标签、样本守恒、独立 IC/Rank IC 复算、累计和及含截距 OLS。浏览器测试覆盖单文件离线加载全部 102 张图、因子筛选、六主题切换与键盘导航、区间选择、人工相位示意、下载、图片放大和手机宽度，禁止离线页发出 HTTP 请求；另检查站点展示版同源图片。
