...
交付文件为 `content/daily/2026-09-06.html`，全部图表和汇总数据内嵌，约 4.8 MB，可直接离线阅读。站点通过同日期 Markdown 日报与 `.show.html` 展示入口发现报告；展示版从 `content/assets/cancel-phase-2026-09-06/` 加载同源图片。
...
- 本地补算展示所需的完整十分组、原值分布和配对定义数值差。分组使用成对有效样本的平均秩百分位，不拆并列值；每日组内股票等权、区间内有效交易日等权。
...
`report.json` 保存汇总数字、逐日日度 IC 和报告审计信息，不含股票代码或逐笔行情。78 张静态图使用 Matplotlib 绘制并保存为 WebP，十分组交互图使用同一份已核验的区间汇总数据，可导出 SVG 与 CSV。
...
数值测试覆盖并列值、缺失标签、样本守恒及验收汇总。浏览器测试覆盖单文件离线加载全部 78 张图、因子筛选、区间选择、人工相位示意、下载、图片放大和手机宽度，禁止离线页发出 HTTP 请求；另检查站点展示版同源图片。
...
图表区间选择更新指标、覆盖率和十分组；日度、月度和原值分布保留全历史，并在图注中明示。研究结果封存状态、阶段 revision、实测时间与执行／样本外限制均放在报告附录。报告仓库的提交不改变封存研究数值。
